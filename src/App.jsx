  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const findValue = (row, keyName) => {
        const key = Object.keys(row).find(k => k.toLowerCase().trim() === keyName.toLowerCase());
        return row[key] || '';
      };
      ...
      const contentData = resourceData.filter(row => {
        if (!filterProject || filterProject === "All Projects") return true;
        const projectValue = findValue(row, 'Project') || findValue(row, 'Client') || '';
        return projectValue.toLowerCase().includes(filterProject.toLowerCase().trim());
      });

      if (contentData.length === 0) {
        throw new Error(`Data Mismatch: Found ${uniqueDates.length} days of logs for ${filterName}, but 0 rows matched the project "${filterProject}".`);
      }
      ...
      const aiPayload = contentData.map(r => ({
        ...
        A: findValue(r, 'Planned Activity for the Day') || findValue(r, 'Activity') || r._col5,
      ...
      <main className="max-w-6xl mx-auto space-y-10">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 animate-fade-in">
          <div>
            ...
            <p className="text-slate-500 mt-2 font-medium">Enterprise performance intelligence & analytics</p>
          </div>
          ...
          <div className="flex items-center gap-3 no-print">
            ...
            <Button variant="premium" onClick={generateReport} disabled={loading}>
            ...
            >
              <Card className="border-indigo-100 bg-white/50 backdrop-blur-sm">
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-slate-700">Sheet Tab Name</label>
                    <Input 
                      value={sheetName} 
        ...
        <Card className="glass-card border-none shadow-xl overflow-visible">
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 overflow-visible">
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-600 flex items-center gap-2"><Calendar size={14} /> Start Date</label>
              <Input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-600 flex items-center gap-2"><Calendar size={14} /> End Date</label>
              <Input type="date" value={toDate} onChange={e => setToDate(e.target.value)} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-600 flex items-center gap-2"><User size={14} /> Resource</label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-600 flex items-center gap-2"><Briefcase size={14} /> Project (Optional)</label>
              <Select value={filterProject} onValueChange={val => setFilterProject(val)}>
                <SelectTrigger>
                  <SelectValue placeholder="Select Project" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="All Projects">All Projects</SelectItem>
                  {PROJECT_LIST.map(proj => <SelectItem key={proj} value={proj}>{proj}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </CardContent>
          ...
          {loading ? (
            ...
            >
              ...
              <div className="space-y-1">
                <p className="text-xl font-bold text-slate-800">AI is distilling your impact...</p>
                <p className="text-slate-500">Synthesizing logs into professional milestones</p>
              </div>
            </motion.div>
          ) : report ? (
            <motion.div 
            ...
            >
              ...
              >
                <div className="border-b border-slate-100 pb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                  <div className="space-y-2">
                    <h2 className="text-5xl font-bold tracking-tighter text-slate-900 line-clamp-1">Performance Insight</h2>
                    <p className="text-indigo-600 font-bold uppercase tracking-[0.3em] text-sm">
                      {new Date(fromDate).toLocaleString('default', { month: 'long', year: 'numeric' })} Report
                    </p>
                  </div>
                  <div className="text-sm text-slate-500 space-y-1 md:text-right">
                    <p className="flex items-center md:justify-end gap-2">Manager: <span className="text-slate-900 font-bold">{DEFAULT_MANAGER}</span></p>
                    <p>ID: GA-{Math.random().toString(36).substr(2, 6).toUpperCase()}</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  <div className="space-y-8">
                    <h3 className="text-sm font-bold uppercase tracking-widest text-slate-400">KPI Summary</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100/50">
                        <p className="text-slate-500 text-xs font-bold uppercase mb-1">Active Days</p>
                        <p className="text-4xl font-black text-slate-800 tracking-tighter">{report.summary.totalDays}</p>
                      </div>
                      <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100/50">
                  ...
                  <div className="space-y-8">
                    <h3 className="text-sm font-bold uppercase tracking-widest text-slate-400">Work Portfolio</h3>
                    <div className="space-y-6 pt-2">
                      {Object.entries(report.summary.distribution).map(([key, value]) => (
                        <div key={key} className="space-y-2">
                          <div className="flex justify-between text-sm font-semibold">
                            <span className="text-slate-600 uppercase text-[11px] tracking-wider">{key}</span>
                            <span className="text-slate-900">{value}%</span>
                          </div>
                          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                            <motion.div initial={{ width: 0 }} animate={{ width: `${value}%` }} transition={{ duration: 1.5, ease: "easeOut" }} className="h-full bg-gradient-to-r from-indigo-600 to-indigo-400" />
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div key={section.id} className="space-y-6 group">
                      ...
                      <ul className="space-y-4">
                        {section.items.map((item, i) => (
                          <li key={i} className="flex gap-4 text-[13px] text-slate-500 leading-relaxed items-start group-hover:text-slate-700 transition-colors">
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-bold text-slate-800">No report generated yet</h3>
                <p className="text-slate-500 max-w-sm mx-auto">Configure your source and filter parameters to synthesize your performance insights.</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black flex items-center gap-3">
              <Mail className="text-indigo-600" /> Professional Broadcast Draft
            </DialogTitle>
          </DialogHeader>
